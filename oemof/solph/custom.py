    ...
    * Default attribute `min` of in/outflows is overwritten by -1 if not set
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(self.inputs) > 1 or len(self.outputs) > 1:
            raise ValueError(
                "Component ElectricLine must not have more than \
    ...
    **The following variable are created:**
    ...
    def _create(self, group=None):
        ...
        if True not in [b.slack for b in self.ELECTRICAL_BUSES]:
            ...
            logging.info(
                "No slack bus set,setting bus {0} as slack bus".format(bus.label)
            )
            bus.slack = True
            ...
            for t in m.TIMESTEPS:
                for n in group:
                    ...
                    try:
                        ...
                        raise ValueError(
                            "Error in constraint creation", "of node {}".format(n.label)
                        )
        ...
        self._equate_electrical_flows = Constraint(group, m.TIMESTEPS, noruleinit=True)
...
class Link(Transformer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(self.inputs) > 2 or len(self.outputs) > 2:
            raise ValueError(
                "Component `Link` must not have more than \
...
class LinkBlock(SimpleBlock):
    ...
    **The following constraints are created:**
    ...
    def _create(self, group=None):
        ...
        all_conversions = {}
        for n in group:
            all_conversions[n] = {k: v for k, v in n.conversion_factors.items()}
...
class GenericCAESBlock(SimpleBlock):
    ...
    def _create(self, group=None):
        ...
        self.cmp_p = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # Compression: Max. Capacity
        self.cmp_p_max = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # Compression: Heat flow
        self.cmp_q_out_sum = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # Compression: Waste heat
        self.cmp_q_waste = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)
        ...
        self.exp_p = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # Expansion: Max. Capacity
        self.exp_p_max = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # Expansion: Heat flow of natural gas co-firing
        self.exp_q_in_sum = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # Expansion: Heat flow of natural gas co-firing
        self.exp_q_fuel_in = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # Expansion: Heat flow of additional firing
        self.exp_q_add_in = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # Cavern: Filling levelh
        self.cav_level = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # Cavern: Energy inflow
        self.cav_e_in = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # Cavern: Energy outflow
        self.cav_e_out = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # TES: Filling levelh
        self.tes_level = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # TES: Energy inflow
        self.tes_e_in = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # TES: Energy outflow
        self.tes_e_out = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # Spot market: Positive capacity
        self.exp_p_spot = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)

        # Spot market: Negative capacity
        self.cmp_p_spot = Var(self.GENERICCAES, m.TIMESTEPS, within=NonNegativeReals)
        ...
        def cmp_q_out_sum_constr_rule(block, n, t):
            return (
                self.cmp_q_out_sum[n, t] == self.cmp_q_waste[n, t] + self.tes_e_in[n, t]
            )
        ...
        def cav_e_in_constr_rule(block, n, t):
            return (
                self.cav_e_in[n, t]
                == n.params["cav_e_in_m"] * self.cmp_p[n, t] + n.params["cav_e_in_b"]
            )
        ...
        def cav_e_out_constr_rule(block, n, t):
            return (
                self.cav_e_out[n, t]
                == n.params["cav_e_out_m"] * self.exp_p[n, t] + n.params["cav_e_out_b"]
            )
        ...
        def cav_eta_constr_rule(block, n, t):
            if t != 0:
                ...
                return n.params["cav_eta_temp"] * self.cav_level[
                    n, t
                ] == m.timeincrement[t] * (self.cav_e_in[n, t] - self.cav_e_out[n, t])
        ...
        def tes_eta_constr_rule(block, n, t):
            if t != 0:
                return self.tes_level[n, t] == self.tes_level[
                    n, t - 1
                ] + m.timeincrement[t] * (self.tes_e_in[n, t] - self.tes_e_out[n, t])
            else:
    ...
    **The following constraints are created:**
    ...
    def _create(self, group=None):
        ...
        def _relation_rule(block, n, t):
            ...
            expr += m.flow[list(n.inputs.keys())[0], n, t] * n.coefficients[1][t]
            expr += (
