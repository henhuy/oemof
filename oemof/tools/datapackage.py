...
def read_facade(
    facade, facades, create, typemap, data, objects, sequence_names, fks, resources
):
...
def deserialize_energy_system(cls, path, typemap={}, attributemap={}):
    ...
    default_typemap = {"bus": Bus, "hub": Bus, DEFAULT: Component, FLOW_TYPE: HSN}
    ...
    data["buses"] = {
        name: create(
            ...
            if mapping
                ...
                ValueError,
                "Typemap is missing a mapping for '{}'.".format(bus.get("type", "bus")),
            ),
            {"label": name},
            bus["parameters"],
        )
    ...
    data["components"] = {
        name: create(
            ...
            resolve_object_references(element["parameters"], f=lambda r: r == "buses"),
        )
    ...
    for r in package.resources:
        if all(
            re.match(r"^data/elements/.*$", p) for p in listify(r.descriptor["path"], 1)
        ):
            r.read(keyed=True)
            foreign_keys = {
                fk["fields"]: fk["reference"]
    ...
    if lst[1:] == lst[:-1]:
        ...
        es.add(
            *chain(
                ...
                facades.values(),
                chain(
                    *[f.subnodes for f in facades.values() if hasattr(f, "subnodes")]
                ),
            )
        )
