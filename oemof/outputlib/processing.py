    ...
    if hasattr(om, "dual"):
        ...
        for bus, timesteps in grouped:
            ...
            if (bus, None) not in result.keys():
                result[(bus, None)] = {"sequences": df, "scalars": pd.Series()}
            else:
...
def __separate_attrs(system, get_flows=False, exclude_none=True):
    ...
    def detect_scalars_and_sequences(com):
        ...
        attrs = [i for i in dir(com) if not (callable(i) or i.startswith(exclusions))]
...
def param_results(system, exclude_none=True):
    ...
    msg = (
        "The function 'param_results' has been renamed to"
