...
es = EnergySystem(timeindex=pandas.date_range("2016-01-01", periods=2, freq="H"))
...
class Economic_Tests:
    def test_diesel_cost_results(self):
        eq_(cost_results[(dg, None)].get("invest"), None)
        eq_(cost_results[(dg, b_el1)]["variable_costs"], 62.5 * 2 * 1)
        eq_(cost_results[(dg, b_el1)]["invest"], 62.5 * 0.5)
        eq_(cost_results[(b_diesel, dg)]["variable_costs"], 31.25 * 2 * 2)

    def test_storage_cost_results(self):
        eq_(cost_results[(batt, None)]["invest"], 600 * 0.4)
        eq_(cost_results[(b_el1, batt)]["variable_costs"], 62.5 * 2 * 3)
        eq_(cost_results[(batt, b_el2)]["variable_costs"], 100 * 2.5)

    def test_custom_costs(self):
        custom_costs = [
            economics.Costs(
                name="real_invest_costs",
                flow_key=economics.AttributeKey(dimension="scalars", name="invest"),
                param_key=economics.AttributeKey(
                    dimension="scalars", name="real_invest_costs"
                ),
            )
        ]
        ...
        eq_(custom_cost_results[(batt, None)]["real_invest_costs"], 600 * 30)

    def test_lcoe_per_node(self):
        diesel_lcoe = economics.calculate_lcoe(dg, results, cost_results)
        output = 125
        ...
        output = 100
        storage_lcoe = economics.calculate_lcoe(batt, results, cost_results)
