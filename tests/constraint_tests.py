...
class Constraint_Tests:
    ...
    def compare_lp_files(self, filename, ignored=None, my_om=None):
        ...
        with open(ospath.join(self.tmppath, tmp_filename)) as generated_file:
            with open(
                ospath.join(
                    ospath.dirname(ospath.realpath(__file__)), "lp_files", filename
                )
            ) as expected_file:
                ...
                expected = remove(
                    ignored, chop_trailing_whitespace(expected_file.readlines())
                )
                generated = remove(
                    ignored, chop_trailing_whitespace(generated_file.readlines())
                )
    ...
    def test_max_source_min_sink(self):
        ...
        solph.Source(
            label="wind",
            outputs={bel: solph.Flow(nominal_value=54, max=(0.85, 0.95, 0.61))},
        )
    ...
    def test_fixed_source_variable_sink(self):
        ...
        solph.Source(
            ...
            outputs={
                bel: solph.Flow(
                    actual_value=[0.43, 0.72, 0.29], nominal_value=10e5, fixed=True
                )
            },
        )
    ...
    def test_invest_source_fixed_sink(self):
        ...
        solph.Sink(
            ...
            inputs={
                bel: solph.Flow(
                    actual_value=[0.5, 0.8, 0.3], nominal_value=10e4, fixed=True
                )
            },
        )
    ...
    def test_storage_invest_5(self):
        ...
        solph.components.GenericStorage(
            label="storage5",
            inputs={
                bel: solph.Flow(investment=solph.Investment(ep_costs=99, existing=110))
            },
            outputs={bel: solph.Flow(investment=solph.Investment(existing=100))},
    ...
    def test_emission_constraints(self):
        ...
        solph.Source(
            label="source1", outputs={bel: solph.Flow(nominal_value=100, emission=0.5)}
        )
        solph.Source(
            label="source2", outputs={bel: solph.Flow(nominal_value=100, emission=0.8)}
        )

        # Should be ignored because the emission attribute is not defined.
        solph.Source(label="source3", outputs={bel: solph.Flow(nominal_value=100)})
    ...
    def test_flow_without_emission_for_emission_constraint(self):
        ...
        def define_emission_limit():
            ...
            solph.Source(
                label="source1",
                outputs={bel: solph.Flow(nominal_value=100, emission=0.8)},
            )
            solph.Source(label="source2", outputs={bel: solph.Flow(nominal_value=100)})
            om = self.get_om()
    ...
    def test_flow_without_emission_for_emission_constraint_no_error(self):
        ...
        bel = solph.Bus(label="electricityBus")
        solph.Source(
            label="source1", outputs={bel: solph.Flow(nominal_value=100, emission=0.8)}
        )
        solph.Source(label="source2", outputs={bel: solph.Flow(nominal_value=100)})
        om = self.get_om()
        solph.constraints.emission_limit(om, limit=777)
    ...
    def test_equate_variables_constraint(self):
        ...
        sink = solph.Sink(
            label="Sink",
            inputs={bus1: solph.Flow(investment=solph.Investment(ep_costs=500))},
        )
        source = solph.Source(
            label="Source",
            outputs={bus1: solph.Flow(investment=solph.Investment(ep_costs=123))},
        )
        om = self.get_om()
