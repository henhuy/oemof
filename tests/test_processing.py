...
class Parameter_Result_Tests:
    ...
    def setUpClass(cls):
        cls.period = 24
        cls.es = EnergySystem(
            timeindex=pandas.date_range("2016-01-01", periods=cls.period, freq="H")
        )
        ...
        dg = Transformer(
            ...
            outputs={
                b_el1: Flow(variable_costs=1, investment=Investment(ep_costs=0.5))
            },
        ...
        demand = Sink(
            label="demand_el",
            inputs={
                b_el2: Flow(nominal_value=1, actual_value=cls.demand_values, fixed=True)
            },
        )
    ...
    def test_flows_without_none_exclusion(self):
        b_el2 = self.es.groups["b_el2"]
        demand = self.es.groups["demand_el"]
        param_results = processing.parameter_as_dict(self.es, exclude_none=False)
        scalar_attributes = {
            "fixed": True,
            "integer": None,
            "investment": None,
        ...
        assert_series_equal(
            param_results[(b_el2, demand)]["scalars"], pandas.Series(scalar_attributes)
        )
