...
class Analyzer_Tests:
    def setup(self):
        self.results = processing.results(es_with_invest.optimization_model)
        self.param_results = processing.param_results(es_with_invest.optimization_model)
        self.analysis = analyzer.Analysis(
            self.results, self.param_results, iterator=analyzer.FlowNodeIterator
        )
    ...
    def test_bus_balance_analyzer(self):
        ...
        eq_(nb.result[es_with_invest.b_diesel]["output"][es_with_invest.dg], 62.5)

        # b_el1:
        eq_(len(nb.result[es_with_invest.b_el1]["input"]), 1)
        eq_(len(nb.result[es_with_invest.b_el1]["output"]), 1)
        eq_(nb.result[es_with_invest.b_el1]["input"][es_with_invest.dg], 125)
        eq_(nb.result[es_with_invest.b_el1]["output"][es_with_invest.batt], 125)
        ...
        eq_(nb.result[es_with_invest.b_el2]["output"][es_with_invest.demand], 100)
    ...
    def test_invest_analyzer(self):
        ...
        # dg-b_el1-Flow
        eq_(invest.result[(es_with_invest.dg, es_with_invest.b_el1)], 62.5 * 0.5)
    ...
    def test_lcoe_analyzer(self):
        ...
        eq_(lcoe.result[(es_with_invest.b_diesel, es_with_invest.dg)], 125 / output)

        # batt
        eq_(lcoe.result[(es_with_invest.batt, None)], 600 * 0.4 / output)
        eq_(lcoe.result[(es_with_invest.b_el1, es_with_invest.batt)], 375 / output)
        eq_(lcoe.result[(es_with_invest.batt, es_with_invest.b_el2)], 250 / output)
        eq_(lcoe.result[(es_with_invest.b_el2, es_with_invest.demand)], 0 / output)
