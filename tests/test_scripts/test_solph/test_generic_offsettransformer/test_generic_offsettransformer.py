...
def test_gen_ostf():
    ...
    solph.Source(label="source_th", outputs={bth: solph.Flow(variable_costs=1000)})
    ...
    solph.Source(
        label="source_el", outputs={bel: solph.Flow(variable_costs=data["price_el"])}
    )
    ...
    hp = solph.custom.OffsetTransformer(
        ...
        coefficients=[[0 for i in range(0, periods)], [2 for i in range(0, periods)]],
    )
