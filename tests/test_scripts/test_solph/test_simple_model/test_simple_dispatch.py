...
import os
import pandas as pd
from oemof.solph import Sink, Source, Transformer, Bus, Flow, Model, EnergySystem
...
def test_dispatch_example(solver="cbc", periods=24 * 5):
    ...
    demand_el = Sink(
        label="demand_elec",
        inputs={
            bel: Flow(nominal_value=85, actual_value=data["demand_el"], fixed=True)
        },
    )
    ...
    demand_th = Sink(
        label="demand_therm",
        inputs={
            bth: Flow(nominal_value=40, actual_value=data["demand_th"], fixed=True)
        },
    )
