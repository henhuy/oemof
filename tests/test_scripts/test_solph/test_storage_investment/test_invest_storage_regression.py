...
def test_regression_investment_storage(solver="cbc"):
    ...
    solph.Sink(
        ...
        inputs={
            bel: solph.Flow(
                actual_value=[209643, 207497, 200108, 191892],
                fixed=True,
                nominal_value=1,
            )
        },
    ...
    solph.components.GenericStorage(
        label="storage",
        inputs={
            bel: solph.Flow(investment=solph.Investment(existing=625046 / 6, maximum=0))
        },
        outputs={
            bel: solph.Flow(investment=solph.Investment(existing=104174.33, maximum=1))
        },
        capacity_loss=0.00,
        initial_capacity=0,
