...
def test_tuples_as_labels_example(filename="storage_investment.csv", solver="cbc"):
    ...
    # Sinks
    solph.Sink(label=Label("sink", "electricity", "excess"), inputs={bel: solph.Flow()})

    solph.Sink(
        label=Label("sink", "electricity", "demand"),
        inputs={
            bel: solph.Flow(actual_value=data["demand_el"], fixed=True, nominal_value=1)
        },
    )
    ...
    solph.Source(
        ...
        outputs={bgas: solph.Flow(nominal_value=194397000 * 400 / 8760, summed_max=1)},
    )
    ...
    solph.Source(
        ...
        outputs={
            bel: solph.Flow(actual_value=data["pv"], nominal_value=582000, fixed=True)
        },
    )
    ...
    stor_invest_dict = {
        ...
        (("storage_electricity_battery", "bus_electricity_",), "flow"): 0,
    }
