...
class EnergySystem_Tests:
    ...
    def test_that_None_is_not_a_valid_group(self):
        ...
        ungrouped = [Entity(uid="Not in 'Group': {}".format(i)) for i in range(10)]
    ...
    def test_grouping_filter_parameter(self):
        g1 = Grouping(
            key=lambda e: "The Special One", filter=lambda e: "special" in str(e)
        )
        g2 = Nodes(key=lambda e: "A Subset", filter=lambda e: "subset" in str(e))
    ...
    def test_proper_filtering(self):
        ...
        """
        g = Nodes(
            key="group", value=lambda _: set((1, 2, 3, 4)), filter=lambda x: x % 2 == 0
        )
    ...
    def test_Flows(self):
        ...
        node = Node(label="A Node", inputs={bus: flows[0]}, outputs={bus: flows[1]})
    ...
    def test_FlowsWithNodes(self):
        ...
        bus = NewBus(label="A Bus")
        node = Node(label="A Node", inputs={bus: flows[0]}, outputs={bus: flows[1]})
        eq_(ES.groups[key], set(((bus, node, flows[0]), (node, bus, flows[1]))))
